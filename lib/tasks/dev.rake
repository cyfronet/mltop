if Rails.env.local?
  namespace :dev do
    ...
    task seed: "db:setup" do
      ...
      t2m = ModelType.find_or_create_by(name: "Text to movie", from: :text, to: :movie)
      m2m = ModelType.find_or_create_by(name: "Movie to movie", from: :movie, to: :movie)
      m2t = ModelType.find_or_create_by(name: "Movie to text", from: :movie, to: :text)
