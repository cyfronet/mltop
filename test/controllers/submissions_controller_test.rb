...
class SubmissionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "fails to create new submission with missing consents" do
    consent = create(:consent, target: "model", required: true)
    assert_no_difference "Model.count" do
      post submissions_path, params: {
        model: { name: "new model", task_ids: [ tasks("st").id, tasks("asr").id ], agreements_attributes: [ { agreementable_type: "Model", consent_id: consent.id, agreed: "0" } ] }
      }
    end
  ...
  test "create new submission with marked agreement" do
    ...
    assert_difference "Agreement.count", 1 do
      assert_difference "Model.count", 1 do
        post submissions_path, params: {
          model: { name: "new model", task_ids: [ tasks("st").id, tasks("asr").id ], agreements_attributes: [ { agreementable_type: "Model", consent_id: consent.id, agreed: "1" } ] }
        }
        assert_response :redirect
      end
    end
